<?php
	...
	if($_SERVER['REQUEST_METHOD'] == "POST"){
		PujolMetabox_updateAttachmentForm();
  }
...
?>
<div id="PujolMetabox_manage" class='wrap'>
	<?php echo "<h2>" . __( 'Assign Metaboxes', 'PujolMetaboxes_trdom' ) . "</h2>"; ?>
	<form method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
		<p class="submit PujolMetaboxes_submit">
    	<input type="submit" name="Submit" class="button-primary" value="<?php _e('Update', 'PujolMetaboxes_trdom' ) ?>" />  
 	 </p>
		...
		<table id="<?php echo $post_type->name;?>" class="table table-condensed table-bordered">
          ...
          if( $my_query->have_posts() ) {
	          while ($my_query->have_posts()) : $my_query->the_post(); ?>
	            ...
	            <?php $col = 0; foreach($metaboxes as $metabox ){
								$applied_to_all = get_option("apply_to_all_".$post_type->name."_".$metabox->id);
								$has = get_post_meta(get_the_ID(),"Pujol_metabox_".$metabox->id,true);
	            	...
	            	?>
							<td>
								<input 
								class='metabox_check' 
								name="Pujol_metabox_<?php echo get_the_ID();?>_<?php echo $metabox->id;?>" 
								data-metabox="<?php echo $metabox->id; ?>" 
...
<?php 
	function PujolMetabox_updateAttachmentForm(){
		global $wpdb;
      ...
      if( $my_query->have_posts() ) {
        while ($my_query->have_posts()) : $my_query->the_post();
        	foreach($metaboxes as $metabox){
        		$field = "Pujol_metabox_".get_the_ID()."_".$metabox->id;
        		if(!empty($_POST[$field])){
        			update_post_meta(get_the_ID(),"Pujol_metabox_".$metabox->id,"1");
        		}else{
        			delete_post_meta(get_the_ID(),"Pujol_metabox_".$metabox->id);
        		}
        	}
        endwhile;
      }
